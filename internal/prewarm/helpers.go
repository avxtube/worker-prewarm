...
// getDomainSettingString reads the grouped domain_setting used by the new
// platform. Legacy single-value documents remain a fallback during rollout.
func getDomainSettingString(ctx context.Context, field, legacyName string) string {
	if setting := getSetting(ctx, "domain_setting"); setting != nil {
		if values, ok := asBsonM(setting.Value); ok {
			if value, ok := values[field].(string); ok && strings.TrimSpace(value) != "" {
				return strings.TrimSpace(value)
			}
		}
	}
	return getSettingString(ctx, legacyName)
}
