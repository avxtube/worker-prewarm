...
// workerId ("prewarm_hostname@n"). The platform node-api enqueuer counts
...
func StartHeartbeat(ctx context.Context, workerID, version string) {
	...
	beat := func() {
		...
		setFields := bson.M{
			...
			"type":         workerType,
			"version":      version,
			"capabilities": []string{"cdn-prewarm", "multi-pop", "sprite-vtt"},
			// enqueuer จัดคิวราย pop + ประทับ target ให้ worker ที่ผูก storage
			"pop":        config.AppConfig.Pop,
			"storageId":  optionalString(config.AppConfig.StorageId),
			"status":     status,
